...
{
    client = new TcpClient(QHostAddress(QString("127.0.0.1")),10000);
    ...
    connect(interface,SIGNAL(stateRequest(EEvents,void*)),
            this,SLOT(toModel(EEvents,void*)));
    connect(interface,SIGNAL(clear()),this,SLOT(clear()));
    connect(interface,SIGNAL(single()),this,SLOT(single()));
    connect(interface,SIGNAL(mode(bool)),this,SLOT(mode(bool)));
    interface->show();
    generator = new TEventGenerator();
    connect(generator,SIGNAL(timeout()),this,SLOT(single()));
}
...
{
    qDebug()<<"clear command";
    toModel(RESET,nullptr);
}

void TApplication::single()
{
    qDebug()<<"single car";
